...
var _ = Describe("TypedSubscriber", func() {
	...
	Describe("notificationMap", func() {
		...
		It("should return error when policy is set for single subscriber", func() {
			_, err := nMap.newSubscriber(sID, handler, SubscriberWithSelectionPolicy(events.MakeSelectionPolicyByValue(events.CountingWindow, 0, 0, time.Now(), time.Nanosecond, time.Nanosecond)))
